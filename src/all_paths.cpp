...
AllPaths::AllPaths(unordered_map<string, unordered_map<string, Route>> &graph, string source,
                   string destination) {
    this->graph = graph;
    this->source = source;
...
bool AllPaths::Path::operator<(const Path &other) const { return this->cost < other.cost; }
void AllPaths::displayAllPaths() {
    for (int i = 0; i < allPathsVector.size(); i++) {
        cout << "Path # " << i + 1 << "\n";
        cout << "The cost is: " << allPathsVector[i].first << '\n';
        cout << "The route:\n";
        for (auto &j : allPathsVector[i].second) cout << j;
        cout << '\n';
    }
}
void AllPaths::dfsAllPaths(string node, map<string, bool> &curVis, vector<string> curPath,
                           int weight) {
    map<transportations, string> enumToStr = {
        {BUS, "Bus"}, {MICROBUS, "Microbus"}, {TRAIN, "Train"}, {METRO, "Metro"}, {UBER, "Uber"}};
    ...
    for (auto &it : this->graph[node])  // city, Route
    {
        ...
        for (auto &i : it.second.roads) {
            if (curVis[it.first])
                continue;
            string tmp = "From " + node + " to " + it.first + " by " + enumToStr[i.transport] +
                         " with cost: " + to_string(i.cost) + " " + "\n";
